...
export async function GET(request: NextRequest) {
  ...
  const rate = await getAnalyticsRateLimit().limit(`analytics:v2:${userId}`);
  ...
  if (rangeError) return NextResponse.json({ error: rangeError, code: ANALYTICS_PLAN_REQUIRED }, { status: 403 });
  try {
