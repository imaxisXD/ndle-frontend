...
import { CheckCircle2Icon, AlertCircleIcon, RefreshCwIcon, SparklesIcon, MousePointerClickIcon, X } from "./icons"
    ...
    <div className="relative">
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute right-0 top-12 z-50 w-96 max-w-[calc(100vw-2rem)] rounded-lg border border-border bg-card shadow-xl">
            ...
            <div className="max-h-96 overflow-y-auto">
              {notifications.length === 0 ? (
                <div className="p-8 text-center">
                  ...
                  <p className="mt-2 font-mono text-sm text-muted-foreground">No notifications</p>
                </div>
              ) : (
                      ...
                      <div className="flex gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-2">
                            <h4 className="font-mono text-sm font-medium">{notification.title}</h4>
                            {!notification.read && <span className="h-2 w-2 flex-shrink-0 rounded-full bg-blue-600" />}
                          </div>
                          <p className="mt-1 font-mono text-xs text-muted-foreground">{notification.message}</p>
                          {notification.link && (
                            <code className="mt-2 inline-block font-mono text-xs text-blue-600">
                              {notification.link}
                            </code>
                          )}
                          <p className="mt-2 font-mono text-xs text-muted-foreground">{notification.timestamp}</p>
                        </div>
                        <button
                          onClick={(e) => {
                            e.stopPropagation()
