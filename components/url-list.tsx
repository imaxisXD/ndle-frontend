...
const mockUrls: Link[] = [
  {
    ...
    memory: {
      ...
      notes: "Great resource for the team meeting next week. Focus on the pricing section.",
  ...
  },
  {
    ...
    status: "checking",
    memory: {
      summary: "Enterprise pricing details including custom features, dedicated support, and SLA guarantees.",
...
export function UrlList() {
  ...
  const [activeTab, setActiveTab] = useState<"memory" | "chat" | "healing">("memory")
      ...
      <div className="border-b border-border p-6">
        ...
        {showFilters && (
          <div className="mt-4 flex flex-wrap gap-3 p-4 rounded-lg bg-muted/30 border border-border">
            <div className="flex items-center gap-2">
              <span className="font-mono text-xs font-medium text-muted-foreground">Status:</span>
              <div className="flex gap-2">
                {(["all", "healthy", "healed", "checking"] as const).map((status) => (
                  <button
                    key={status}
            ...
            <div className="flex items-center gap-2 ml-auto">
              ...
              <span className="font-mono text-xs font-medium text-muted-foreground">Sort by:</span>
              <select
                value={sortBy}
        ...
        {(searchQuery || statusFilter !== "all") && (
          <div className="mt-3 flex items-center gap-2 flex-wrap">
            <span className="font-mono text-xs text-muted-foreground">Active filters:</span>
            {searchQuery && (
              ...
              >
                <div className="flex-1 space-y-2">
                  <div className="flex items-center gap-3">
                    ...
                    <code className="font-mono text-sm font-medium text-foreground">{url.shortUrl}</code>
                    <button
                  ...
                  <p className="font-mono text-xs text-muted-foreground">{url.originalUrl}</p>
                </div>

                <div className="flex items-center gap-6">
                  <div className="text-right">
                    <p className="font-mono text-sm font-medium">{url.clicks}</p>
                    <p className="font-mono text-xs text-muted-foreground">clicks</p>
                  </div>
                  <div className="text-right">
                    <p className="font-mono text-xs text-muted-foreground">{url.created}</p>
                  </div>
                  <button
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="rounded-lg border border-border bg-card p-4">
                          <div className="mb-2 flex items-center gap-2">
                            ...
                            <h4 className="font-mono text-sm font-medium">Your Notes</h4>
                          </div>
                          <p className="font-mono text-sm text-muted-foreground">{url.memory.notes}</p>
                        </div>

                        <div className="rounded-lg border border-yellow-200 bg-yellow-50 p-4">
                          <div className="mb-2 flex items-center gap-2">
                            <ClockIcon className="h-4 w-4 text-yellow-700" />
                            <h4 className="font-mono text-sm font-medium text-yellow-900">Why You Saved This</h4>
                          </div>
                          <p className="font-mono text-sm text-yellow-700">{url.memory.savedReason}</p>
                        </div>
                      </div>
                    )}
                      ...
                      <div className="space-y-4">
                        {url.conversations && url.conversations.length > 0 ? (
                            ...
                            {url.conversations.map((conv) => (
                              <div key={conv.id} className="space-y-3">
                                <div className="rounded-lg bg-muted p-4">
                                  <p className="font-mono text-sm text-foreground">{conv.question}</p>
                                  <p className="mt-1 font-mono text-xs text-muted-foreground">{conv.timestamp}</p>
                                </div>
                                <div className="rounded-lg border border-border bg-card p-4">
                                  <div className="mb-2 flex items-center gap-2">
                                  ...
                                  <p className="font-mono text-sm text-foreground leading-relaxed">{conv.answer}</p>
                                </div>
                              </div>
                            ))}
                            <div className="rounded-lg border border-border bg-card p-4">
                          ...
                          <div className="rounded-lg border border-dashed border-border bg-muted/30 p-8 text-center">
                            ...
                            <p className="mt-2 font-mono text-sm text-muted-foreground">No conversations yet</p>
                      ...
                      <div className="space-y-3">
                        {url.healingHistory && url.healingHistory.length > 0 ? (
                            ...
                            <div key={idx} className="flex gap-4">
                              <div className="flex flex-col items-center">
                                <div className="rounded-full bg-muted p-2">
                                  {event.action.includes("404") || event.action.includes("Detected") ? (
                                    <AlertCircleIcon className="h-3.5 w-3.5 text-orange-600" />
                                  ) : event.action.includes("Found") || event.action.includes("semantic") ? (
                                    <RefreshCwIcon className="h-3.5 w-3.5 text-yellow-600" />
                                  ) : (
                                    <ArchiveIcon className="h-3.5 w-3.5 text-blue-600" />
                                  )}
                                </div>
                                {idx < url.healingHistory!.length - 1 && <div className="h-full w-px bg-border" />}
                              </div>
                              <div className="flex-1 pb-4">
                                <p className="font-mono text-sm font-medium">{event.action}</p>
                                <p className="mt-1 font-mono text-xs text-muted-foreground">{event.date}</p>
                                {event.from && (
                                  <p className="mt-2 font-mono text-xs text-muted-foreground">From: {event.from}</p>
                                )}
                                {event.to && (
                                  <p className="mt-1 font-mono text-xs text-muted-foreground">To: {event.to}</p>
                                )}
                              </div>
                            </div>
                          ))
                          ...
                          <div className="rounded-lg border border-dashed border-border bg-muted/30 p-8 text-center">
                            ...
                            <p className="mt-2 font-mono text-sm text-foreground">Link is healthy</p>
                            <p className="mt-1 font-mono text-xs text-muted-foreground">No healing actions required</p>
                          </div>
                        )}
                      </div>
                    )}
