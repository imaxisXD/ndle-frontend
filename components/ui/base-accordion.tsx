...
const accordionPanelVariants = cva(
  "text-sm text-accent-foreground overflow-hidden",
  {
    variants: {
      variant: {
        default: "",
...
function AccordionPanel(props: React.ComponentProps<typeof Accordion.Panel>) {
  ...
  return (
    <Accordion.Panel
      ...
      className={cn(
        accordionPanelVariants({ variant }),
        "grid transition-[grid-template-rows] duration-200 ease-in-out",
        "grid-rows-[0fr]",
        // Support both data-state (standard) and data-panel-open (legacy/Base UI specific?)
        "data-[panel-open]:grid-rows-[1fr] data-[state=open]:grid-rows-[1fr]",
        className,
      )}
      {...rest}
    >
      <div className={cn("min-h-0 overflow-hidden", "pt-0 pb-5")}>
        {children}
      </div>
    </Accordion.Panel>
  );
