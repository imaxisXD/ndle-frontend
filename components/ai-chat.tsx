  ...
  const handleSend = async () => {
    ...
    setTimeout(() => {
      const aiMessage: Message = {
        id: Math.random().toString(36).substring(7),
        role: "assistant",
        content: `Based on the content at ${linkUrl}, here's what I found: ${getMockResponse(input)}`,
        timestamp: "Just now",
      }
    ...
    <div className="space-y-4">
      {messages.length === 0 ? (
        <div className="rounded-lg border border-dashed border-border bg-muted/30 p-8 text-center">
          ...
          <p className="mt-2 font-mono text-sm text-foreground">Ask AI about this link</p>
          <p className="mt-1 font-mono text-xs text-muted-foreground">
            Get instant answers about the content, key points, or specific details
          </p>
        </div>
      ) : (
              ...
              {message.role === "assistant" ? (
                <div className="rounded-lg border border-border bg-card p-4">
                  <div className="mb-2 flex items-center gap-2">
                    ...
                    <span className="font-mono text-xs font-medium text-muted-foreground">AI Assistant</span>
                  </div>
                  <p className="font-mono text-sm text-foreground leading-relaxed">{message.content}</p>
                  <p className="mt-2 font-mono text-xs text-muted-foreground">{message.timestamp}</p>
                </div>
              ) : (
                <div className="rounded-lg bg-muted p-4 max-w-[80%]">
                  <p className="font-mono text-sm text-foreground">{message.content}</p>
                  <p className="mt-1 font-mono text-xs text-muted-foreground">{message.timestamp}</p>
                </div>
              )}
            </div>
          ))}
          {isLoading && (
            <div className="rounded-lg border border-border bg-card p-4">
              <div className="mb-2 flex items-center gap-2">
                ...
                <span className="font-mono text-xs font-medium text-muted-foreground">AI is thinking...</span>
              </div>
